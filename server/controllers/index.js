  ...
  */
  try {
    ...
    */
    ...
    return res.render('page1', { cats: docs, pageName: 'Page 1' });
  } catch (err) {
...
const hostPage2 = (req, res) => {
  res.render('page2', { pageName: 'Page 2' });
};

// Function to render the untemplated page3.
const hostPage3 = (req, res) => {
  res.render('page3', { pageName: 'Page 3' });
};

const hostPage4 = async (req, res) => {
  try {
    const docs = await Dog.find({}).lean().exec();
    return res.render('page4', { pageName: 'page 4', dogs: docs });
  } catch (err) {
    console.log(err);
...
const createDog = async (req, res) => {
  ...
  // create instance of dog using dog model
  const newDog = new Dog(dogData);

  // try to store in db
  try {
    // save - ( creates new or updates accordingly )
    await newDog.save();
    return res.status(201).json({
      name: newDog.name,
      breed: newDog.breed,
...
const findAndAgeDog = async (req, res) => {
  if (!req.body.name) return res.status(500).json({ error: 'name is required to perform a search' });
  let doc = {};
  try {
  ...
  if (!doc) return res.status(404).json({ error: 'dog not found' });
